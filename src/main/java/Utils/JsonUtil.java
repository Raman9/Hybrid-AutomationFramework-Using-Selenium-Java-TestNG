...
public class JsonUtil {
	...
	public static void loadData() {
		if (users == null) {
			...
			try {
				...
				InputStream inputStream = JsonUtil.class.getClassLoader().getResourceAsStream("users.json");

				if (inputStream == null) {
					throw new RuntimeException(
							"CRITICAL ERROR: Could not find 'testdata/users.json'. Check file name and location.");
				}
	...
	public static ProductData getProduct(String productId) {
		if (products == null) {
			ObjectMapper mapper = new ObjectMapper();
			try {
				InputStream inputStream = Json.class.getClassLoader().getResourceAsStream("products.json");
	...
	public static void bankInfo() {
		if (users == null) {
			...
			try {
				...
				InputStream inputStream = JsonUtil.class.getClassLoader().getResourceAsStream("bankInfo.json");
